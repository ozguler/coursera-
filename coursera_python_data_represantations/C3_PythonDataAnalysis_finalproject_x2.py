...
#   print(top_players)
    player_info = read_csv_as_list_dict("/Users/ozguler/Downloads/gitrepo/coursera_python_data_represantations/Master2016.csv".csv,","," ")
...
#   Create a dict of dicts - outer dict has the key playerid
#   inner dict has the keys as fields list and values will be manipulated

#   Assign values to the dict of dict

#   Now find the stat rows that correspond to our playerid
#   and update our dict of dict values as the sum of individual stats

#   Let's create a list of unique player names
#    players = []
#    for row in statistics:
#        if (row["playerID"]) in players:
#            print("in players list!")
#            continue
#        else:
#            print("not in players list!")
#            players.append(row["playerID"])

#    for playerid in players:
#        agg_stats[playerid] = {}
#        for x in range(0,len(fields)):
#            agg_stats[playerid][fields[x]] = 0

#    for playerid in players:   #run the loop per unique userid
#        print(playerid)
#        for row in statistics: #for each unique userid go through  .xls
#            print(row)
#            if row["playerID"] == playerid:
#           print row
#                for x in range(0,len(fields)):
#                print(type(agg_stats[playerid][fields[x]]))
#                print(type(row[fields[x]]))
#                    if (row[fields[x]]) == "":
#                        row[fields[x]] = 0
                    #if the field statistic is non zero add it to this players list of stats
                    #each corresponding list member, so that we have an aggregate statistics
                    #for each field for that particular user
#                    agg_stats[playerid][fields[x]] += int(row[fields[x]])
...
#    print(statistics[2].keys())
    agg_stats = []
    players = []
#    agg_stats.append(aggregate_by_player_id(statistics,statistics[0]["playerID"],["G","AB"]))
#    agg_stats.append(aggregate_by_player_id(statistics,statistics[1]["playerID"],["G","AB"]))
#    agg_stats.append(aggregate_by_player_id(statistics,statistics[2]["playerID"],["G","AB"]))
#    agg_stats.append(aggregate_by_player_id(statistics,statistics[3]["playerID"],["G","AB"]))
...
#        else:
#            print("in players list")
#            agg_stats.append(aggregate_by_player_id(statistics,row["playerID"],battingfields))
#        players.append(row["playerID"])
...
#        print(x)
#        print((agg_stats[x]))
    top_players = top_player_ids(info, agg_stats, formula, numplayers)
#    print(top_players)
...
#statistics = read_csv_as_list_dict("/Users/ozguler/Downloads/gitrepo/coursera_python_data_represantations/Batting_2016.csv",","," ")
#print(statistics)
#x = filter_by_year(statistics, 1979, 1979 )
#print(x)
...
#tpid = top_player_ids(info, x, batting_average, 10)
#print(tpid)

#x  = read_csv_as_list_dict("/Users/ozguler/Downloads/gitrepo/coursera_python_data_represantations/Master_2016.csv",","," ")
#lookup_player_names(x, tpid)

#statistics = read_csv_as_list_dict("battingfile",","," ")
#compute_top_stats_year(info, onbase_percentage, 5, 1982)

#aggregate_by_player_id(statistics,"allisar01", ["AB","R","H","2B","3B","HR","SH","GIDP"])

#aggregate_by_player_id(info, batting_average, 3)
compute_top_stats_career(info, batting_average, 3 )
